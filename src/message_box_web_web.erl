...
loop(Req) ->
	...
	{redirect,Url} ->
	    Req:respond({302, 
                         ...
                         ""});
        {send_data, ContentType, ContentDisposition, Data} ->
	    Req:respond({200,
	...
	{static, File} ->
	    "/" ++ StaticFile = File,
	    Req:serve_file(StaticFile,message_box_web_deps:local_path(["www"]));

	{error, ErrorCode, DetailText} when is_integer(ErrorCode) ->
	    Req:respond({ErrorCode,[],DetailText});
	{error, Reason} ->
            ?debugVal(Reason),
...
render_template(ViewFile,Data,Env) -> 
    ...
    FullPathToFile = filename:join([message_box_web_deps:local_path(["views"]),ViewFile]),
